...
function ChatContainer({ onBack }) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/40 backdrop-blur-lg">
      {/* Enhanced ChatHeader with back button for mobile */}
      <div className="relative">
        {onBack && (
          <button
            onClick={onBack}
            className="absolute left-2 top-1/2 transform -translate-y-1/2 md:hidden p-2 rounded-lg bg-slate-700/50 hover:bg-slate-600/50 transition-colors z-10"
          >
                ...
                >
                  {msg.image && (
                    <img
                      src={msg.image}
                      alt="Shared"
                      className="rounded-lg max-h-48 md:max-h-56 w-full object-cover mb-2"
                    />
                  )}
                  {msg.text && <p className="leading-relaxed text-sm md:text-[15px]">{msg.text}</p>}
                  <p className="text-xs mt-1 md:mt-2 opacity-70 text-right">
